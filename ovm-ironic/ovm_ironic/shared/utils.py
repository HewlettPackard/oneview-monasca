...
def extract_oneview_uuid(oneview_uri):
    """Function to extract the uuid associated to OV resource

    :param oneview_uri:  url with the uuid from the OV resource
    :returns The uuid for the OV resource
    """
    ov_uuid = oneview_uri.split('/')[-1]
    return ov_uuid


def extract_ironic_uuid(node):
    """Function to extract the uuid associated to Ironic resource

    :param node:  Dict from Ironic node
    :returns The uuid for the Ironic resource
    """
    return node.__dict__['uuid']
